...
面向 LLM 后训练实习作品集的工程级 **On-Policy Distillation（OPD）** 项目。当前执行固定单训练 seed、GPU 成本最低的最小可信方案：8B Student、14B Teacher、OpenR1-Math-220k、Verifier + Teacher-confidence 加权蒸馏。

完整实验设计与 GPU 预算见 [`PROJECT_PLAN.md`](PROJECT_PLAN.md)。更大规模的备用方案保存在 [`plans/`](plans/README.md)。

## 研究问题

在相同 Student、训练 token 和 Teacher annotation 预算下，使用数学 Verifier 过滤样本，并根据 Teacher token entropy 调整蒸馏损失，能否比 SFT 和等权 Vanilla OPD 获得更好的数学推理质量—成本表现？
...
| Vanilla OPD | 对 Student rollout 做等权 token-level 蒸馏 |
| Verifier-only OPD | 按 `pass/unknown/fail` 加权 |
| Confidence-only OPD | 按 Teacher entropy 加权 token |
| Weighted OPD | 同时使用 Verifier 与 confidence |

所有训练使用唯一预注册 seed `42`。Round 1 只运行 Vanilla 和 Weighted 的最佳 Round 0 checkpoint。
...
prepare + deduplicate + contamination audit
        │
        ▼
8B Student on-policy rollout (vLLM, resumable shards)
        ├──────────────► Math verifier: pass/fail/unknown
        │
        ▼
14B Teacher forcing annotation
top-k log-probs + tail mass + token entropy
        │
        ▼
rebuildable training views
        │
        ├── SFT
        ├── Vanilla OPD
        ├── Verifier OPD
...
- **质量加权**：Verifier 控制 sample loss；归一化 Teacher-confidence 控制 token loss。
...
- **成本硬停止**：`opd budget` 汇总 GPU hours，达到 `110 H100h` 时阻止训练脚本继续启动。
...
```text
prepare → contamination audit → rollout → verify
→ teacher annotate → build weighted view → train → evaluate
...
Round 1 前先把 PEFT adapter 合并为 vLLM checkpoint：
...
LoRA 模型必须先用 `scripts/merge_checkpoint.sh` 合并，Base 则可直接传 `Qwen/Qwen3-8B`，包装器会自动使用配置中的固定 revision。脚本通过 `opd benchmark run` 调用 LightEval，并自动写入命令、job metrics 和 checksum manifest。任务注册表位于 [`eval/registry.yaml`](eval/registry.yaml)。项目锁定 `lighteval==0.9.2`；正式运行前仍需核对 GPQA 访问权限。若设置 `LIGHEVAL_TASKS`，其值应是逗号分隔的注册表名称，例如 `math500,aime2024`。
